/**
 * TypeScript definitions for AWS::CloudWatch::Alarm
 ...
 */
export namespace AWS_CloudWatch_Alarm {
  /**
   ...
   * Any expression used must return a single time series. For more information, see Metric Math Syntax and Functions in the *User Guide*.
   */
  ...
  export type AlarmResourceType = {
    ...
    /**
     * Sets how this alarm is to handle missing data points. Valid values are `breaching`, `notBreaching`, `ignore`, and `missing`. For more information, see Configuring How Alarms Treat Missing Data in the *Amazon User Guide*.
     ...
     * For a list of namespaces for metrics from AWS services, see Services That Publish Metrics.
     */
    Namespace?: string;
    ...
    /**
     * The number of datapoints that must be breaching to trigger the alarm. This is used only if you are setting an "M out of N" alarm. In that case, this value is the M, and the value that you set for `EvaluationPeriods` is the N value. For more information, see Evaluating an Alarm in the *User Guide*.
