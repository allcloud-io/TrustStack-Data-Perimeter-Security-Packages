...
export type Lambda_PermissionSecurityStackProps = cdk.StackProps &
  Readonly<{
    securityPackagesDir: string;
    config: LambdaPermissionSecurityConfig;
  }>;

/**
 * Stack for the Lambda Permission Security Package.
 */
export class Lambda_PermissionSecurityStack extends cdk.Stack {
  private readonly props: Lambda_PermissionSecurityStackProps;
  ...
  constructor(
    scope: Construct,
    id: string,
    props: Lambda_PermissionSecurityStackProps,
  ) {
    ...
    this.packageConfigSSMParameter = new ssm.StringParameter(
      ...
      {
        parameterName:
          "/trust-stack/lambda/permission-security/config" satisfies SharedSSMParameterName,
  ...
  private createProactiveControl() {
    return new CloudFormationLambdaHook(this, "ProactiveControl", {
      ...
      description:
        "CloudFormation hook handler to validate Lambda permission configurations",
      hookExecutionRole: this.hookExecutionRole,
      targetOperations: ["RESOURCE"],
      targetNames: ["AWS::Lambda::Permission"],
      failureMode: "FAIL",
  ...
  private createDetectiveControl() {
    const lambdaHandler = createNodejsLambdaFunction(
      ...
      "DetectiveControlHandler",
      {
        ...
        description:
          "Monitor CloudTrail events for Lambda permission creation and validate " +
    ...
    new events.Rule(this, "LambdaAPIAddPermissionRule", {
      eventPattern: {
        source: ["aws.lambda"],
  ...
  private createResponsiveControl() {
    const lambdaHandler = createNodejsLambdaFunction(
      ...
      {
        functionName: `${SECURITY_PACKAGE_NAME}-RemoveUnauthorizedPermissions`,
        description:
        ...
        initialPolicy: [
          new iam.PolicyStatement({
            sid: "AllowGetLambdaPolicyAndRemovePermission",
            effect: iam.Effect.ALLOW,
    ...
    // when Security Hub findings for unauthorized Lambda permissions are created
    new events.Rule(this, "LambdaUnauthorizedPermissionRule", {
      eventPattern: {
